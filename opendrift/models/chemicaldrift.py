...
class ChemicalDrift(OceanDrift):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._add_config({
            ...
            'chemical:dynamic_partitioning': {'type': 'bool', 'default': True,
                                              ...
                                              'description': 'Toggle dynamic partitioning'},
            ...
            'chemical:compound': {'type': 'enum',
                                  'enum': ['Naphthalene', 'Phenanthrene', 'Fluoranthene',
                                           ...
                                           'Copper', 'Cadmium', 'Chromium', 'Lead', 'Vanadium', 'Zinc', 'Nickel',
                                           'Nitrogen', None],
                                  'default': None,
                                  'level': self.CONFIG_LEVEL_ESSENTIAL, 'description': ''},
        })
        ...
        if transfer_setup == 'organics':
            ...
            if diss == 'nondiss':
                ...
                if diss == 'acid':
                    ...
                    KOC_DOM = (0.08 * ((Phi_n_water * (KOC_DOM_n)) + ((1 - Phi_diss_water) * 10 ** (np.log10(
                        KOW) - 3.5)))) / 0.526  # from  http://i-pie.org/wp-content/uploads/2019/12/ePiE_Technical_Manual-Final_Version_20191202
            ...
            self.transfer_rates = np.zeros(
                [len(self.salinity_intervals), self.transfer_rates.shape[0], self.transfer_rates.shape[1]])
        ...
        if transfer_setup == 'metals' or \
            ...
            if transfer_setup == 'organics' and diss == 'nondiss':
                ...
                self.elements.transfer_rates1D[self.elements.specie == self.num_humcol, self.num_lmm] = \
                    self.k21_0 / tempcorrDOM[self.elements.specie == self.num_humcol] / salinitycorr[
                        self.elements.specie == self.num_humcol]

                self.elements.transfer_rates1D[self.elements.specie == self.num_prev, self.num_lmm] = \
                    self.k31_0 / tempcorrSed[self.elements.specie == self.num_prev] / salinitycorr[
                        self.elements.specie == self.num_prev]

                self.elements.transfer_rates1D[self.elements.specie == self.num_srev, self.num_lmm] = \
                    self.k41_0 / tempcorrSed[self.elements.specie == self.num_srev] / salinitycorr[
                        self.elements.specie == self.num_srev]
                ...
                KOC_watcorrSPM = self.calc_KOC_watcorrSPM(KOC_SPM_initial, KOC_sed_n, pKa_acid, pKa_base, KOW,
                                                          pH_water_SPM, diss)
                KOC_watcorrDOM = self.calc_KOC_watcorrDOM(KOC_DOM_initial, KOC_DOM_n, pKa_acid, pKa_base, KOW,
                                                          pH_water_DOM, diss)
                ...
                self.elements.transfer_rates1D[self.elements.specie == self.num_humcol, self.num_lmm] = \
                    self.k21_0 * KOC_watcorrDOM / tempcorrDOM[self.elements.specie == self.num_humcol] / salinitycorr[
                        self.elements.specie == self.num_humcol]

                self.elements.transfer_rates1D[self.elements.specie == self.num_prev, self.num_lmm] = \
                    self.k31_0 * KOC_watcorrSPM / tempcorrSed[self.elements.specie == self.num_prev] / salinitycorr[
                        self.elements.specie == self.num_prev]

                self.elements.transfer_rates1D[self.elements.specie == self.num_srev, self.num_lmm] = \
                    self.k41_0 * KOC_sedcorr / tempcorrSed[self.elements.specie == self.num_srev] / salinitycorr[
                        self.elements.specie == self.num_srev]
            ...
            if transfer_setup == 'metals':
                ...
                sed_phi = self.get_config('chemical:sediment:corr_factor')  # sediment correction factor
                sed_poro = self.get_config('chemical:sediment:porosity')  # sediment porosity
                sed_H = self.get_config(
                    'chemical:sediment:layer_thickness')  # thickness of seabed interaction layer (m)
            ...
            if self.get_config('chemical:species:Sediment_reversible'):
                ...
                interaction_thick = self.get_config(
                    'chemical:sediment:layer_thickness')  # thickness of seabed interaction layer (m)
    ...
    def update_partitioning(self):
        ...
        phaseshift = np.array(
            self.num_elements_active() * [False])  # Denotes which trajectory that shall be transformed
        ...
        for iin in range(self.nspecies):
            for iout in range(self.nspecies):
                self.ntransformations[iin, iout] += sum(
    ...
    def sorption_to_sediments(self, sp_in=None, sp_out=None):
        ...
        if self.get_config('chemical:species:LMM'):
            self.elements.z[(sp_out == self.num_srev) & (sp_in == self.num_lmm)] = \
                -1. * self.environment.sea_floor_depth_below_sea_level[
                    (sp_out == self.num_srev) & (sp_in == self.num_lmm)]
        ...
        if self.get_config('chemical:species:LMMcation'):
            self.elements.z[(sp_out == self.num_srev) & (sp_in == self.num_lmmcation)] = \
                -1. * self.environment.sea_floor_depth_below_sea_level[
                    (sp_out == self.num_srev) & (sp_in == self.num_lmmcation)]
    ...
    def desorption_from_sediments(self, sp_in=None, sp_out=None):
        ...
        if self.get_config('chemical:species:LMM'):
            self.elements.z[(sp_out == self.num_lmm) & (sp_in == self.num_srev)] = \
                -1. * self.environment.sea_floor_depth_below_sea_level[
                    (sp_out == self.num_lmm) & (sp_in == self.num_srev)] + desorption_depth
        ...
        if self.get_config('chemical:species:LMMcation'):
            self.elements.z[(sp_out == self.num_lmmcation) & (sp_in == self.num_srev)] = \
                -1. * self.environment.sea_floor_depth_below_sea_level[
                    (sp_out == self.num_lmmcation) & (sp_in == self.num_srev)] + desorption_depth
    ...
    def update_chemical_diameter(self, sp_in=None, sp_out=None):
        ...
        logger.debug('Updated particle diameter for %s elements' % len(
            self.elements.diameter[(sp_out == self.num_prev) & (sp_in != self.num_prev)]))
    ...
    def resuspension(self):
        ...
        if self.get_config('chemical:slowly_fraction'):
            self.ntransformations[self.num_ssrev, self.num_psrev] += sum(
                (resusp) & (self.elements.specie == self.num_ssrev))
            self.elements.specie[(resusp) & (self.elements.specie == self.num_ssrev)] = self.num_psrev

        if self.get_config('chemical:irreversible_fraction'):
            self.ntransformations[self.num_sirrev, self.num_pirrev] += sum(
                (resusp) & (self.elements.specie == self.num_sirrev))
    ...
    def update(self):
        ...
        # Resuspension
        self.resuspension()
        logger.info('partitioning: {} {}'.format([sum(self.elements.specie == ii) for ii in range(self.nspecies)],
                                                 self.name_species))
    ...
    def write_netcdf_chemical_density_map(self, filename, pixelsize_m='auto', zlevels=None,
        ...
        self.conc_topo = reader_sea_depth.get_variables_interpolated_xy(['sea_floor_depth_below_sea_level'],
                                                                        ...
                                                                        )[0]['sea_floor_depth_below_sea_level'].reshape(
            self.conc_lon.shape)
        ...
        nc.variables['specie'].long_name = ' '.join(
            ['{}:{}'.format(isp, sp) for isp, sp in enumerate(self.name_species)])
        ...
        if 0:
            ...
            nc.variables['concentration'].long_name = self.get_config(
                'chemical:compound') + ' concentration ' + '\n' + 'specie ' + \
                                                      ' '.join(['{}:{}'.format(isp, sp) for isp, sp in
                                                                enumerate(self.name_species)])
        ...
        if horizontal_smoothing:
            ...
            nc.variables['concentration_smooth'].long_name = self.get_config(
                'chemical:compound') + ' horizontally smoothed concentration ' + '\n' + 'specie ' + \
                                                             ' '.join(['{}:{}'.format(isp, sp) for isp, sp in
                                                                       enumerate(self.name_species)])
            ...
            nc.variables['concentration_smooth'].comment = 'Smoothed over ' + str(
                smoothing_cells) + ' grid points in all horizontal directions'
        ...
        if time_avg_conc:
            ...
            nc.variables['concentration_avg'].long_name = self.get_config(
                'chemical:compound') + ' time averaged concentration ' + '\n' + 'specie ' + \
                                                          ' '.join(['{}:{}'.format(isp, sp) for isp, sp in
                                                                    enumerate(self.name_species)])
    ...
    def emission_factors(self, scrubber_type, chemical_compound):
        ...
        if scrubber_type == "open_loop":
            ...
            Emission_factors = 1e6 * emission_factors_AFP.get(chemical_compound)[
                0]  # 1g = 1e6 ug: AFP is expressed as g
